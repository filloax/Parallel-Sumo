...
$VenvFolder = Join-Path -Path $Root -ChildPath ".conda"
if (-not (Test-Path -Path $VenvFolder -PathType Container)) {
    conda create --yes --prefix=$VenvFolder python=3.10
    conda activate $VenvFolder
    conda install --yes --file $Root/Scripts/requirements.txt
    conda install --yes -c conda-forge pymetis 
}
